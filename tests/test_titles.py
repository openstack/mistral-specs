...
class TestTitles(testtools.TestCase):
    ...
    def _check_lines_wrapping(self, filename, raw):
        code_block = False

        for i, line in enumerate(raw.split("\n")):
            # Allow code block lines to be longer than 79.
            if code_block:
    ...
    def _check_no_cr(self, filename, raw):
    ...
    def _check_trailing_spaces(self, filename, raw):
        for i, line in enumerate(raw.split("\n")):
            ...
            self.assertEqual(
                0,
                len(trailing_spaces),
                "Found trailing spaces on line %s of %s" % (i + 1, filename)
            )
        ...
        for release in releases:
            ...
            for filename in files:
                self.assertTrue(
                    ...
                    "spec %s must use 'rst' extension." % filename
                )
